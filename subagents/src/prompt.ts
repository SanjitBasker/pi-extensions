...
export const SUBAGENT_SPAWN_TOOL_DESCRIPTION =
  "Spawn a background subagent: a fully autonomous, headless pi session with its own context window and this environment's normal host permissions. Fire-and-forget: this returns immediately with an id. The subagent's final output is queued back to you as a message when it settles, or collect it explicitly with subagent_wait. Children cannot orchestrate more agents/workflows or ask the user, and cannot see this conversation, so the prompt must be self-contained. Only use trusted working directories. Max 4 subagents can be running at once.";
...
export const SUBAGENT_SPAWN_PROMPT_GUIDELINES = [
  ...
  "Subagents run on the pi harness and inherit the current model and thinking level unless explicitly overridden.",
...
export const SUBAGENT_SPAWN_PARAMETER_DESCRIPTIONS = {
  ...
  harness:
    'Subagent harness: always "pi" (in-process pi session; inherits this environment).',
  workingDir:
    "Trusted working directory for the autonomous child (default: current working directory)",
  model:
    'Model hint for pi: "provider/model-id" or a model id. Omit to inherit the current model.',
  reasoningEffort:
    "Pi thinking level override. Omit to inherit the current level.",
};
