 ...
 * these types. Pi session events are translated into the normalized
 * `SubagentEvent` union.
 */
...
/**
 * Shared reasoning-effort scale, passed directly to the pi backend.
 * Omitted = the parent session's thinking level.
 */
...
export interface SpawnTask {
  ...
  readonly cwd: string;
  /**
   * Model hint: "provider/model-id" or a bare model id. Omitted = inherit
   * the parent model.
   */
  readonly model?: string;
  /** Pi thinking level override. */
...
export interface SubagentMeta {
  ...
  /** Native pi session id, when available. */
