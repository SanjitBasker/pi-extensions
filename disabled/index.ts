...
const CONFIG_PATH = join(homedir(), ".config", "pi_extensions", "external_memory.toml");
...
function parseExternalMemoryToml(text: string): ExternalMemoryConfig {
	...
	const flush = () => {
		if (section === "project" && current.name && (current.memory_root || current.memoryRoot)) {
			projects.push({ name: current.name, memoryRoot: current.memory_root ?? current.memoryRoot });
		} else if (section === "mapping" && (current.directory || current.path) && current.project) {
			mappings.push({ directory: current.directory ?? current.path, project: current.project });
		}
		current = {};
	};
	...
	for (const rawLine of text.split(/\r?\n/)) {
		...
		if (line === "[[mapping]]" || line === "[[mappings]]" || line === "[[directory_mapping]]") {
			flush();
			section = "mapping";
			continue;
		}
...
function resolveActiveProject(cwd: string): ActiveProject | undefined {
	const projectByName = new Map(config.projects.map((project) => [project.name, project]));
...
function projectRelativePath(filePath: string, project: ActiveProject): string | undefined {
...
function memoryFilesForProjectPath(projectRelativeFile: string, project: ActiveProject): string[] {
...
function formatMemory(files: string[], intro: string): string {
	const sections = files.map((filePath) => `## ${filePath}\n\n${readFileSync(filePath, "utf8").trim()}\n`);
	return `# External Memory\n\n${intro}\n\n${sections.join("\n")}`.trim();
}

function markTouched(pi: ExtensionAPI, project: ActiveProject, relativePath: string): void {
...
function markSent(pi: ExtensionAPI, project: ActiveProject, memoryPath: string, forPath: string): void {
...
function unsentMemoryForPaths(paths: string[], project: ActiveProject): string[] {
	const files: string[] = [];
...
export default function (pi: ExtensionAPI) {
	...
	pi.on("before_agent_start", async (event) => {
		...
		for (const file of files) markSent(pi, activeProject, file, "before-agent-start");
	...
	pi.on("tool_result", async (event): Promise<ToolResultEventResult | undefined> => {
		...
		if (isReadToolResult(event)) rawPath = pathFromReadEvent(event);
			...
			rawPath = typeof event.input.path === "string" ? event.input.path : undefined;
		}
