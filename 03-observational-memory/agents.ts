...
export async function runMemoryAgent(opts:{ctx:any;model:any;auth?:any;thinking:string;maxTurns:number;system:string;prompt:string;tool:AgentTool<any>;signal?:AbortSignal}){
 ...
 const abort=()=>agent.abort();opts.signal?.throwIfAborted();opts.signal?.addEventListener("abort",abort,{once:true});try{await agent.prompt(opts.prompt)}finally{opts.signal?.removeEventListener("abort",abort)}const last=[...agent.state.messages].reverse().find((m:any)=>m.role==="assistant") as any;if(last?.stopReason==="error")throw new Error(last.errorMessage||"memory agent failed");return agent.state.messages;
}
export const observationSchema=Type.Object({observations:Type.Array(Type.Object({timestamp:Type.String(),content:Type.String(),relevance:StringEnum(["low","medium","high","critical"] as const),sourceEntryIds:Type.Array(Type.String())}))});
